...
#
# subscription_id here is the STATE subscription (where the tfstate
# storage account lives, per _fleet.yaml.state.subscription_id), not
# acr.subscription_id. If omitted, the azurerm backend falls back to
# ARM_SUBSCRIPTION_ID or az-CLI context, which can silently target the
# wrong subscription under OIDC.
#
