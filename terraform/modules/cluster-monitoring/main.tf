# modules/cluster-monitoring/main.tf
#
# Managed Prometheus wiring for one AKS cluster. PLAN §4 Stage 1:
#
#   - Data Collection Rule (DCR) `dcr-prom-<cluster>` that binds the
#     AKS `azureMonitorProfile.metrics` scrape output to the env-scope
#     Azure Monitor Workspace via the env-scope Data Collection
#     Endpoint (both owned by `bootstrap/environment`; ingestion
#     traffic transits the env Network Security Perimeter).
#   - Two Data Collection Rule Associations on the AKS resource:
#       * `dcr-<cluster>` → binds the DCR above
#       * `configurationAccessEndpoint` → binds the env DCE (needed by
#         the AKS metrics addon to discover its ingestion endpoint)
#   - Three Prometheus recording rule groups authored against the env
#     AMW: `NodeRecordingRules`, `KubernetesRecordingRules`,
#     `UXRecordingRules`. Rule bodies tracked from the kube-prometheus
#     baseline (matching
#     `Azure/terraform-azurerm-avm-res-containerservice-managedcluster//modules/monitoring`
#     upstream v0.5.x). Alerting rules are a separate follow-up
#     (wiring to the env Action Group lives here once the rule bodies
#     land).
#
# Rule-group `location` and `parent_id` are intentionally the cluster's
# resource group + region, not the obs RG; this is what the AKS
# metrics addon expects and matches the reference module's topology.

# --- DCR binding AKS metrics → env AMW via env DCE --------------------------

resource "azapi_resource" "dcr_prom" {
  type      = "Microsoft.Insights/dataCollectionRules@2023-03-11"
  name      = "dcr-prom-${var.cluster_name}"
  ...
  body = {
    kind = "Linux"
    properties = {
      dataCollectionEndpointId = var.dce_id
      dataSources = {
        prometheusForwarder = [
          {
...
# --- DCRAs on the AKS cluster ----------------------------------------------
#
# Two parallel associations authored as children of the AKS resource.
# Both are required by the metrics addon: the DCR association enables
# data flow; the DCE association tells the scraper which endpoint to
# push to.

resource "azapi_resource" "dcra_prom" {
  ...
  body = {
    properties = {
      dataCollectionRuleId = azapi_resource.dcr_prom.id
    }
  }
}

resource "azapi_resource" "dcea_prom" {
  type      = "Microsoft.Insights/dataCollectionRuleAssociations@2023-03-11"
...
#
# Rule sets ported from
# `Azure/terraform-azurerm-avm-res-containerservice-managedcluster//modules/monitoring`
# (v0.5.x). Three groups: node, ux, kubernetes.
...
resource "azapi_resource" "prg_ux" {
  ...
  body = {
    properties = {
      ...
      scopes      = [var.amw_id, var.aks_cluster_id]
      rules = [
        {
