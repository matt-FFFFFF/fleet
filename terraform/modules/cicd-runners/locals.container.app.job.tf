...
locals {
  ...
  sensitive_environment_variables_github = var.version_control_system_authentication_method == "pat" ? [
    ...
    ] : [
    {
      ...
      # Vendor deviation (KV-reference mode): when github_app_key_kv_secret_id
      # is set, emit value=null here so the plaintext never reaches
      # `azurerm_container_app_job.secret.value` (which has no write_only
      # attribute as of azurerm 4.69 and lands in state). The submodule then
      # emits the secret as a keyVaultUrl reference via
      # var.github_app_key_kv_secret_id.
      value                     = var.github_app_key_kv_secret_id != null ? null : var.version_control_system_github_application_key
