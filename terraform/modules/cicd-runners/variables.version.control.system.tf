...
The application key (PEM) for the GitHub App authentication method.
  ...
  validation {
    condition = (
      ...
      (
        ...
        || var.github_app_key_kv_secret_id != null
      ) : true
    )
    error_message = "For authentication_method=github_app supply either version_control_system_github_application_key (plaintext) or github_app_key_kv_secret_id + github_app_key_identity_id (KV-reference)."
  }
}
...
variable "github_app_key_kv_secret_id" {
  ...
  default     = null
  description = <<DESCRIPTION
(Optional, **vendor extension**.) Versionless Key Vault secret URI holding
...
variable "github_app_key_identity_id" {
  ...
  default     = null
  description = <<DESCRIPTION
(Optional, **vendor extension**.) Resource id of the user-assigned managed
