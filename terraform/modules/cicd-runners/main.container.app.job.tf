module "container_app_job" {
  ...
  # Vendor extension: Key Vault-reference for the GitHub App private key.
  # When set, the "application-key" Container Apps secret is emitted with
  # keyVaultUrl + identity instead of an inline value, so the PEM stays in
  # Key Vault and never materialises in Terraform state.
  github_app_key_kv_secret_id = var.github_app_key_kv_secret_id
  github_app_key_identity_id  = var.github_app_key_identity_id
