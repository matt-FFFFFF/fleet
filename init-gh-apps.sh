...
#     written as `"0"` initially; bump it (or rely on a re-run after
#     rotating the PEM) to drive a re-PUT of the KV secret.
...
#
# `fleet_runners_app_pem_version` is written as "0" on first emit. Bump
# it manually if you rotate the PEM out-of-band; a re-run of this script
# after rotation overwrites the file but does NOT auto-increment the
# version (the script has no record of prior versions). The KV secret is
# re-PUT whenever this version string changes.
...
lines = [
    ...
    "#",
    "# Bump fleet_runners_app_pem_version after rotating the PEM to drive",
    "# a re-PUT of the `fleet-runners-app-pem` Key Vault secret.",
    "",
